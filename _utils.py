...
def read_edit_CSN_examples(filename, data_num):
    """Read examples from filename."""
    examples = []
    with open(filename, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            if 'example' in js.keys():
                example = ' '.join(js['example'].strip().split())
                source = nl + ' </s> ' + example
            elif 'skeleton' in js.keys():
                skeleton = ' </s> '.join(js['skeleton'])
                source = nl + ' </s> ' + skeleton
            else:
                source = nl
            examples.append(
                Example(
