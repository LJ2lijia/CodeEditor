...
def get_filenames(data_root, task, sub_task, split=''):
    if task == 'concode':
        ...
        data_dir = '{}/repair/{}'.format(data_root, sub_task)
        train_fn = '{}/train.json'.format(data_dir)
        dev_fn = '{}/valid.json'.format(data_dir)
        test_fn = '{}/test.json'.format(data_dir)
    elif task == 'review':
        data_dir = '{}/review/{}'.format(data_root, sub_task)
        train_fn = '{}/train.json'.format(data_dir)
        dev_fn = '{}/valid.json'.format(data_dir)
        test_fn = '{}/test.json'.format(data_dir)
    elif task == 'translate':
...
def read_examples(filename, data_num, task):
    read_example_dict = {
        ...
        'review': read_review_examples,
        'repair': read_review_examples,
    }
...
#### add by lijia for pre-training
class InputFeatures_PT(object):
...
def convert_examples_to_features_PT(item):
    js, tokenizer, args = item
    
    
    if 'source' in js and 'target' in js:
        source = js['source']
        source_tokens = js['source'].split()
        target = js['target']
        target_tokens = js['target'].split()
    else:
        source_tokens = js['source_tokens']
        target_tokens = js['target_tokens']
        source = ' '.join(js['source_tokens'])
        target = ' '.join(js['target_tokens'])
    
    input_ids = tokenizer.encode(source, max_length=args.max_source_length, padding='max_length', truncation=True)
    target_ids = tokenizer.encode(target, max_length=args.max_target_length, padding='max_length', truncation=True)

    return InputFeatures_PT(source_tokens, input_ids, target_tokens, target_ids)
